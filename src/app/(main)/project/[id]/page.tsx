...
export default async function ProjectPage({
  ...
  // Run both access check and request check in a single authenticated session
  const { projectWithAccess, request } = await withAuthenticatedSession(
    user.id,
    async (tx) => {
      const project = await getProjectsQuery(tx, user.id, null, null, [
        id,
      ]).executeTakeFirst();
      ...
      return { projectWithAccess: project, request: req };
    },
  );
