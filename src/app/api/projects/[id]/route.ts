...
export const PATCH = projectAction(
  async (_req, { project, user, body, role }) => {
    ...
    if (folderId !== undefined) {
      // Only Creator or Owner can move project
      if (role !== "creator" && role !== "owner") {
        throw {
          status: 403,
          message: "Forbidden: Only creator or owner can move project",
        };
      }
    ...
    if (deletedAt === null) {
      ...
      if (role !== "creator") {
        throw { status: 403, message: "Only creator can restore project" };
      }
      await db
        .updateTable("projects")
        .set({ deletedAt: null })
    ...
    if (name || description !== undefined) {
      // Only Creator or Owner can update details
      if (role !== "creator" && role !== "owner") {
        throw {
          status: 403,
          message:
            "Forbidden: Only creator or owner can update project details",
        };
      }
...
export const DELETE = projectAction(async (req, { project, user, role }) => {
  const db = getDb();
  ...
  if (role !== "creator") {
    throw { status: 403, message: "Only creator can delete project" };
  }
