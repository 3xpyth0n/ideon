...
export async function up(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    ...
    await sql`DROP POLICY IF EXISTS folder_collaborators_select ON "folderCollaborators"`.execute(
      db,
    );
    await sql`DROP POLICY IF EXISTS folder_collaborators_write ON "folderCollaborators"`.execute(
      db,
    );
    ...
    await sql`DROP POLICY IF EXISTS block_snapshots_isolation ON "blockSnapshots"`.execute(
      db,
    );
    await sql`
...
export async function down(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    ...
    await sql`DROP FUNCTION IF EXISTS check_folder_access(text, text)`.execute(
      db,
    );
