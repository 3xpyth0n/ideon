...
const TYPES_WITH_FOLDER = `'text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder'`;
const TYPES_WITHOUT_FOLDER = `'text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell'`;
...
export async function up(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    ...
    await sql`ALTER TABLE blocks ADD CONSTRAINT "blocks_blockType_check" CHECK ("blockType" IN ('text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder'))`.execute(
      db,
    );
  } else {
    ...
    await db.transaction().execute(async (trx) => {
      await recreateBlocksTable(trx, TYPES_WITH_FOLDER);
...
export async function down(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    ...
    await db.transaction().execute(async (trx) => {
      await recreateBlocksTable(trx, TYPES_WITHOUT_FOLDER, true);
      await recreateBlockSnapshotsTable(trx);
      await recreateLinkPreviewsTable(trx);
      await recreateBlockReactionsTable(trx);
    });
...
async function recreateBlocksTable(
  ...
  if (excludeFolder) {
    await sql`INSERT INTO blocks SELECT * FROM blocks_old WHERE blockType != 'folder'`.execute(
      trx,
    );
  } else {
