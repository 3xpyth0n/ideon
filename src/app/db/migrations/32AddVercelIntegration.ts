...
const TYPES_WITH_VERCEL = `'text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder', 'vercel'`;
const TYPES_WITHOUT_VERCEL = `'text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder'`;
...
export async function up(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    ...
    await sql`ALTER TABLE blocks ADD CONSTRAINT "blocks_blockType_check" CHECK ("blockType" IN (${sql.raw(
      TYPES_WITH_VERCEL,
    )}))`.execute(db);

    await db.schema
      .createTable("userVercelTokens")
      .addColumn("id", "text", (col) => col.primaryKey())
      .addColumn("userId", "text", (col) =>
        col.notNull().references("users.id").onDelete("cascade").unique(),
      )
      .addColumn("accessToken", "text", (col) => col.notNull())
      .addColumn("authMethod", "text", (col) =>
        col.notNull().defaultTo("oauth"),
      )
    ...
    await db.schema
      .createTable("userVercelProjects")
      .addColumn("id", "text", (col) => col.primaryKey())
      .addColumn("userId", "text", (col) =>
        col.notNull().references("users.id").onDelete("cascade"),
      )
      .addColumn("vercelProjectId", "text", (col) => col.notNull())
      .addColumn("vercelProjectName", "text", (col) => col.notNull())
    ...
    await db.transaction().execute(async (trx) => {
      await recreateBlocksTable(trx, TYPES_WITH_VERCEL);
...
export async function down(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    await db.schema.dropTable("userVercelProjects").ifExists().execute();
    await db.schema.dropTable("userVercelTokens").ifExists().execute();

    await sql`ALTER TABLE blocks DROP CONSTRAINT IF EXISTS "blocks_blockType_check"`.execute(
      db,
    );
    await sql`ALTER TABLE blocks ADD CONSTRAINT "blocks_blockType_check" CHECK ("blockType" IN (${sql.raw(
      TYPES_WITHOUT_VERCEL,
    )}))`.execute(db);
  } else {
    await sql`PRAGMA foreign_keys=OFF`.execute(db);

    await db.transaction().execute(async (trx) => {
      await sql`DROP TABLE IF EXISTS "userVercelProjects"`.execute(trx);
      await sql`DROP TABLE IF EXISTS "userVercelTokens"`.execute(trx);
      await recreateBlocksTable(trx, TYPES_WITHOUT_VERCEL, true);
      await recreateBlockSnapshotsTable(trx);
      await recreateLinkPreviewsTable(trx);
      await recreateBlockReactionsTable(trx);
    });
...
async function recreateBlocksTable(
  ...
  if (excludeVercel) {
    await sql`INSERT INTO blocks SELECT * FROM blocks_old WHERE blockType != 'vercel'`.execute(
      trx,
    );
  } else {
