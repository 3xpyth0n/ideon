...
const TYPES_WITH_FRAME = `'text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder', 'vercel', 'frame'`;
const TYPES_WITHOUT_FRAME = `'text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder', 'vercel'`;
...
export async function up(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    ...
    await db.transaction().execute(async (trx) => {
      await recreateBlocksTable(trx, TYPES_WITH_FRAME);
...
export async function down(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    ...
    await sql`ALTER TABLE blocks ADD CONSTRAINT "blocks_blockType_check" CHECK ("blockType" IN ('text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder', 'vercel'))`.execute(
      db,
    );
  } else {
    await sql`PRAGMA foreign_keys=OFF`.execute(db);
    ...
    await db.transaction().execute(async (trx) => {
      await recreateBlocksTable(trx, TYPES_WITHOUT_FRAME, true);
      await recreateBlockSnapshotsTable(trx);
      await recreateLinkPreviewsTable(trx);
      await recreateBlockReactionsTable(trx);
    });
...
async function recreateBlocksTable(
  ...
  if (excludeFrame) {
    await sql`INSERT INTO blocks SELECT * FROM blocks_old WHERE blockType != 'frame'`.execute(
      trx,
    );
  } else {
