...
const TYPES_WITH_BEHAVIORS = `'text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder', 'vercel', 'frame', 'webhook', 'cron'`;
const TYPES_WITHOUT_BEHAVIORS = `'text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder', 'vercel', 'frame'`;
...
export async function up(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    ...
    await sql`ALTER TABLE blocks ADD CONSTRAINT "blocks_blockType_check" CHECK ("blockType" IN ('text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder', 'vercel', 'frame', 'webhook', 'cron'))`.execute(
      db,
    );
    await sql`ALTER TABLE "automationRules" ADD COLUMN IF NOT EXISTS "sourceBlockId" TEXT REFERENCES blocks(id) ON DELETE CASCADE`.execute(
      db,
    );
  } else {
    ...
    await db.transaction().execute(async (trx) => {
      await recreateBlocksTable(trx, TYPES_WITH_BEHAVIORS);
    ...
    await sql`ALTER TABLE "automationRules" ADD COLUMN "sourceBlockId" TEXT REFERENCES blocks(id) ON DELETE CASCADE`.execute(
      db,
    );
  }
}
...
export async function down(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    ...
    await sql`ALTER TABLE blocks ADD CONSTRAINT "blocks_blockType_check" CHECK ("blockType" IN ('text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder', 'vercel', 'frame'))`.execute(
      db,
    );
    await sql`ALTER TABLE "automationRules" DROP COLUMN IF EXISTS "sourceBlockId"`.execute(
      db,
    );
  } else {
    await sql`PRAGMA foreign_keys=OFF`.execute(db);
    ...
    await db.transaction().execute(async (trx) => {
      await recreateBlocksTable(trx, TYPES_WITHOUT_BEHAVIORS, true);
      await recreateBlockSnapshotsTable(trx);
      await recreateLinkPreviewsTable(trx);
      await recreateBlockReactionsTable(trx);
    });
...
async function recreateBlocksTable(
  ...
  if (excludeBehaviors) {
    await sql`INSERT INTO blocks SELECT * FROM blocks_old WHERE blockType NOT IN ('webhook', 'cron')`.execute(
      trx,
    );
  } else {
