...
const TYPES_WITH_LATEX = `'text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder', 'vercel', 'frame', 'webhook', 'cron', 'latex'`;
const TYPES_WITHOUT_LATEX = `'text', 'link', 'file', 'core', 'github', 'palette', 'contact', 'video', 'snippet', 'checklist', 'kanban', 'sketch', 'shell', 'folder', 'vercel', 'frame', 'webhook', 'cron'`;
...
export async function up(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    ...
    // Create the new table first, copy data, drop original, rename new one.
    // This avoids ALTER TABLE RENAME which SQLite propagates to FK refs in
    // other tables (blockSnapshots, blockReactions, linkPreviews), leaving
    // them with stale references after the old table is dropped.
    await sql`PRAGMA foreign_keys=OFF`.execute(db);
    await db.transaction().execute(async (trx) => {
      await buildBlocksTable(trx, "blocks_next", TYPES_WITH_LATEX);
...
export async function down(db: Kysely<database>): Promise<void> {
  ...
  if (isPostgres) {
    ...
    await sql`PRAGMA foreign_keys=OFF`.execute(db);
    await db.transaction().execute(async (trx) => {
      await buildBlocksTable(trx, "blocks_prev", TYPES_WITHOUT_LATEX);
      await sql`INSERT INTO blocks_prev SELECT * FROM blocks WHERE blockType != 'latex'`.execute(
        trx,
      );
      await sql`DROP TABLE blocks`.execute(trx);
